...
async function loadMyOrders() {
  ...
  if (token) {
    apiUrl = "http://localhost:5000/api/orders/my-orders";
    headers = { Authorization: `Bearer ${token}` };
  } else {
    apiUrl = `http://localhost:5000/api/orders/guest-orders/${guestPhone}`;
  }
...
async function sendCancelRequest() {
  ...
  try {
    const res = await fetch(
      `http://localhost:5000/api/orders/cancel-request/${selectedOrder._id}`,
      {
        method: "POST",
        headers: {
